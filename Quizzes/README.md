...
* Check the `ConnectionStrings` in `appsettings.json` files under the `Quizzes.Blazor` and `Quizzes.DbMigrator` projects and change it if you need.
...
```bash
dotnet dev-certs https -v -ep openiddict.pfx -p 4469872f-dc64-4e22-99c5-0bd419c805ca
```

> `4469872f-dc64-4e22-99c5-0bd419c805ca` is the password of the certificate, you can change it to any password you want.
...
* `Quizzes.DbMigrator`: A console application which applies the migrations and also seeds the initial data. It is useful on development as well as on production environment.
** `Quizzes.Blazor`: ASP.NET Core Blazor Server application that is the essential web application of the solution.
